...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if(fLiteMode) return; // ignore all Btcgreen related functionality
...
{
    if(fLiteMode) return; // disable all Btcgreen specific functionality
    ...
    RenameThread("btcgreen-ps-server");
